...
public class DUUIWorker implements Callable<DUUIWorker>, PipelineWorker {
    ...
    public DUUIWorker call() throws Exception {
        ...
        try {
            ...
            _childrenLocks.forEach(ComponentLock::countDown); // children can continue
                ...
                updatePipelineGraphStatus(_name, _signature.toString(), Color.RED3);
                _childrenLocks.forEach(ComponentLock::fail);
                throw new AnnotatorUnreachableException(this, new RuntimeException(format(
                    "[%s] Pipeline component failed.%n", _threadName), e
                ));
            } finally {
                _total = Duration.ofNanos(System.nanoTime() - start);
                documentUpdate(_name, _signature, "parent_wait", parentWait);
                documentUpdate(_name, _signature, "worker_total", _total.toNanos());
    ...
    private void waitForParents() throws InterruptedException {
        while (_finishedParents.size() != _parentLocks.size()) {
            for (DUUIWorker.ComponentLock parent : _parentLocks) {
                boolean finished = parent.await(1, TimeUnit.MILLISECONDS);
                if (parent.failed() && !finished)
