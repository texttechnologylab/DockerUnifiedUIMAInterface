...
/**
 * the port numbers allowed here should be the same as ports:- xx:xx in docker-compose.yml.
...
public class PortManager {
    ...
    private static int BASE = 20000;
    private static int MAX = 20100;
    ...
    public static void setMode(String str) throws IOException {
        if (str.contains("local")) {
            BASE = ConfigManager.getManager(new File("src/main/resources/slurmDriverConf/slurmDriver.yaml")).getConfig().getPhysicalMode().getLocalCluster().getFree_port_from();
            MAX = ConfigManager.getManager(new File("src/main/resources/slurmDriverConf/slurmDriver.yaml")).getConfig().getPhysicalMode().getLocalCluster().getFree_port_to();

        } else if (str.contains("remote")) {
            BASE = ConfigManager.getManager(new File("src/main/resources/slurmDriverConf/slurmDriver.yaml")).getConfig().getPhysicalMode().getRemoteCluster().getFree_port_from();
            MAX = ConfigManager.getManager(new File("src/main/resources/slurmDriverConf/slurmDriver.yaml")).getConfig().getPhysicalMode().getRemoteCluster().getFree_port_to();
