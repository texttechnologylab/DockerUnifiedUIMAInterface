...
public class DUUISlurmDriver implements IDUUIDriverInterface {
    private SlurmRestContainer _rest;
    ...
    public DUUISlurmDriver(SlurmRestContainer rest) throws IOException, UIMAException, SAXException {
        _rest = rest;
    ...
    public DUUISlurmDriver(SlurmRestPhysical restLocal) throws IOException, UIMAException, SAXException {
        _rest_local = restLocal;
        _interface = new DUUISlurmInterface(restLocal);
        _client = HttpClient.newHttpClient();
...
// 装填好的实例
    ...
    public void shutdown() throws IOException, InterruptedException{
        ...
        if(_rest == null){
            token = _rest_local.generateTokenByHost();
            System.out.println("shutdown hook clean all jobs");
            HashMap<String, String> jobIDPortMap = _interface.get_jobID_PortMap();
            String finalToken = token;
            jobIDPortMap.keySet().forEach(jobID -> {
                try {
                    _rest_local.cancelJob(jobID);
    ...
    public boolean destroy(String uuid) throws IOException, InterruptedException {
        ...
        if(_rest_local == null){
            ...
            if (!comp.getRunningAfterExit()) {
                ...
                for (DUUISlurmDriver.ComponentInstance inst : comp.getInstances()) {
                    ...
                    boolean status = _rest.cancelJob(jobId);
        ...
        if(_rest == null){
            if(!_rest_local.getPassword_token().isEmpty()){
                token = _rest_local.getPassword_token();
            }
            else {
                token = _rest_local.generateTokenByHost();
            }
            ...
            if (!comp.getRunningAfterExit()) {
                ...
                for (DUUISlurmDriver.ComponentInstance inst : comp.getInstances()) {
                    String jobId = inst.get_job_ID();
                    boolean status = _rest_local.cancelJob(jobId);
                    PortManager.release(inst.get_hostPort());
                    _interface.get_jobID_PortMap().remove(jobId);
                    counter += 1;
                    flag = flag && status;
